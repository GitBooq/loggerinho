...
FileSink::FileSink(std::shared_ptr<IFormatter> formatter,
...
bool FileSink::open() const noexcept { return file_.is_open(); }
...
void FileSink::write(const std::string &message) noexcept {
  try {
    if (!file_.is_open()) {
      file_.open(filePath_, std::ios::app);
      if (!file_.is_open()) {
...
void FileSink::deleteOldestArchiveIfExist() {
  std::string oldest =
...
void FileSink::renameArchivesIfAny() {
...
void FileSink::renameCurrentFileToArchive() {
  if (std::filesystem::exists(filePath_)) {
    std::filesystem::rename(filePath_, filePath_.generic_string() + ".1");
  }
}

std::size_t FileSink::getBufferSize(std::stringstream &buffer) {
